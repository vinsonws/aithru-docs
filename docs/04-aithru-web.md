...
Current implementation baseline: `0.1.0` package version, with a v0.2-style local draft key and feature scope in the app.

## One-line definition

```txt
Aithru Web = browser-safe WorkflowSpec editor + node palette + validation + graph editing + simulated run panel
```
...
- real server or desktop bridge APIs
...
A future desktop bridge or server runtime executes workflows.
```
...
| Real local runtime execution | future `aithru-desktop` bridge or `aithru-server` |
| Real HTTP execution | tool executor in runtime composition, not browser UI |
| Durable human approval | desktop/server runtime, not browser simulation |

## Current warnings

The app should keep warning users when a node is present but not truly executable in the browser:

- `core.httpRequest`: metadata only; execution requires desktop bridge or server runtime.
- `core.humanApproval`: simulation only; durable pause/resume requires desktop bridge or server runtime.
...
`aithru-web` should not become the real runtime, credential store, tool executor, or durable approval system.
